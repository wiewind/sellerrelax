    ...
    public static function mkDir ($dir) {
        ...
        foreach($tempPaths as $temp) {
            ...
            if (!file_exists($checkPath)) {
                ...
                if (!self::chmodir($checkPath, 0777)) {
                    throw new Exception(sprintf(__('Error by set permissions [%s].'), $checkPath));
                }
            }
        }
    }
        ...
        if (is_dir($path)) {
            ...
            while (($file = readdir($dh)) !== false) {
                if($file != '.' && $file != '..') {
                    ...
                    if(!self::chmodir($fullpath, $filemode)) {
                        closedir($dh);
                        return false;
                    }
                }
